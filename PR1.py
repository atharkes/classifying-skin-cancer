...
tekst2 = np.loadtxt(data_path, delimiter=',', dtype=str, usecols=range(cols))
a = np.delete(tekst2,0,0)
q = a.astype(int)
...
for i,n in enumerate(train):
    ...
    y_train[i] = y
test = q[(n_train + 1):]
newtrain = np.expand_dims(x_train,axis=2)
x_train2 = newtrain.reshape(n_train,dim1,dim2,dim3)
...
x_test2 = newtest.reshape(n_test,dim1,dim2,dim3)
